...
> **Note:** API version 66.0 is required to support ternary expressions in LWC templates (e.g., `{condition ? 'a' : 'b'}`). Lower versions will fail with LWC1535 errors.
    ...
    <flexiPageRegions>
        <itemInstances>
            <componentInstance>
                ...
                <identifier>c_greenlightDashboard</identifier>
            </componentInstance>
        </itemInstances>
        <name>main</name>
        <type>Region</type>
...
> **Note:** The `<identifier>` element is required. Use the component name with `:` replaced by `_` (e.g., `c:greenlightDashboard` → `c_greenlightDashboard`).
...
> **Note:** `<formFactors>` must be a flat string value (`Large`), not a nested element. Nested `<formFactor>` elements will fail with schema validation errors.
